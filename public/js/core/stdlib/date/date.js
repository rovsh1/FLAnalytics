(function(){
	...
	Date.factory = function(dateEntity) {
		if (dateEntity instanceof Date)
			return dateEntity;
		else if (isString(dateEntity)) {
			if ('' === dateEntity)
				return new Date();
			let date = new Date();
			date.fromString(dateEntity);
			return date;
		} else if (isNumber(dateEntity)) {
			let date = new Date();
...
(function(){
	...
	var getReltext = function(ordinal, currentValue, lastValue) {
		switch (ordinal) {
			case 'this': return currentValue;
			case 'previous': return currentValue - 1;
			case 'next': return currentValue + 1;
			case 'last': return lastValue;
		}
	};
	var getOrdinal = function(ordinal, firstValue) {
		switch (ordinal) {
			case 'first': return firstValue;
			case 'second': return firstValue + 1;
			case 'third': return firstValue + 2;
			case 'fourth': return firstValue + 3;
			case 'fifth': return firstValue + 4;
			case 'sixth': return firstValue + 5;
			case 'seventh': return firstValue + 6;
			case 'eighth': return firstValue + 7;
			case 'ninth': return firstValue + 8;
			case 'tenth': return firstValue + 9;
			case 'eleventh': return firstValue + 10;
			case 'twelfth': return firstValue + 11;
		}
		return false;
	};
	var handlers = {
	...
	var unitHandler = function(unit, ordinal, rootUnit) { //порядковые числительные и указатели
	...
	Date.prototype.fromString = function(string) {
		...
		var m = string.match(/^(\d{2}).(\d{2}).(\d{4})(.*)/);
		if (m) {
...
Date.prototype.isEqual = function(date, format) {
	...
	switch (format) {
		case 'year': return date.format('Y') === this.format('Y');
		case 'month': return date.format('Ymd') === this.format('Ymd');
		case 'hours': return date.format('YmdH') === this.format('YmdH');
		case 'minutes': return date.format('YmdHi') === this.format('YmdHi');
		case 'seconds': return date.format('YmdHis') === this.format('YmdHis');
		default: return date.format('Ymd') === this.format('Ymd');
	}
	return false;
};
