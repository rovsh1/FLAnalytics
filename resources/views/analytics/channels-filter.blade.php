        ...
        gapi.analytics.ready(function() {
            ...
            function renderUrldata() {
                ...
                if((urlBack.indexOf('auto') > -1) || (urlBack.indexOf('tech') > -1 )){
                    ...
                    filter = 'ga:pagePath=@'+urlBack+'\;ga:pagePath!@auto;ga:pagePath!@tech';
                }

                // Adjust `now` to experiment with different days, for testing only...
                var now = moment(); // .subtract(3, 'day');
                var dimensions = ('<?=$dimension?>' !== '') ? 'ga:<?=$dimension?>' : '';
                var sort = '-ga:<?=$dimension?>';
                ...
                if($('#compare')[0].checked == true){
                        ...
                        if(data1 && data1 !== undefined){
                             data1.forEach(function (item,key) {
                                ...
                                var tr1 = "<tr class='date1 child-row' data-addkey='"+(key+1)+"'>" +
                                    ...
                                    "<td class='users'>"+item[1]+"</td>" +
                                    "<td class='new-users'>"+item[2]+"</td>" +
                            ...
                            data2.forEach(function (item,key) {
                                ...
                                var pageviews_td = tr.find('.users');
                                var pageviewsuniq_td = tr.find('.new-users');
                                var sessions = tr.find('.sessions');
                                pageviews_td.text(change_views.toFixed(2)+"%");
                                pageviewsuniq_td.text(change_uniq.toFixed(2)+"%");
                                sessions.text(change_uniq.toFixed(2)+"%");
                    ...
                    Promise.all([first_query]).then(function(results) {
                        ...
                        if(data1 && data1 !== undefined){
                            ...
                            data1.forEach(function (item,key) {
                                total_pageviews+=parseInt(item[1]);
                                total_uniquepageviews+=parseInt(item[2]);
                                total_sessions+=parseInt(item[3]);
                                var tr = "<tr><td>"+(key+1)+"</td><td>"+item[0]+"</td><td>"+item[1]+"</td><td>"+item[2]+"</td><td>"+item[3]+"</td></tr>";
                                tbody.append(tr);
                            });
                            tbody.prepend('<tr><td></td><td>Total</td><td>'+total_pageviews+'</td><td>'+total_uniquepageviews+'</td><td>'+total_sessions+'</td></tr>');
                        }
