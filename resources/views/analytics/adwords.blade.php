                ...
                if($('#compare')[0].checked == true){
                    ...
                    Promise.all([first_query,second_query]).then(function(results) {
                        ...
                        metricsArray.forEach(function (item) {
                            addTotal1+='<td>'+parseFloat(results[0].totalsForAllResults[item]).toFixed(2)+'</td>';
                            addTotal2+='<td>'+parseFloat(results[1].totalsForAllResults[item]).toFixed(2)+'</td>';
                        ...
                        tbody.prepend("<tr>" +
                            "<td></td><td>Total "+results[1].query['start-date']+"-"+results[1].query['end-date']+"</td>"+addTotal2+
                            "<td></td><td></td></tr>");
                        tbody.prepend("<tr>" +
                            "<td></td><td>Total "+results[0].query['start-date']+"-"+results[0].query['end-date']+"</td>"+addTotal1+
                            "<td></td><td></td></tr>");

                        tbody.prepend("<tr>" +
                            "<td>Gain</td>" +
                            "<td></td>" +totalDiff+"<td></td><td></td></tr>");
                    ...
                    Promise.all([first_query]).then(function(results) {
                        ...
                        if(data1 && data1 !== undefined){
                            data1.forEach(function (item,key) {
                                ...
                                item.forEach(function (subitem,subkey) {
                                    if(subkey==0){
                                        let urlFilter = new URL(window.location.href);
                                        urlFilter.searchParams.set('start-date1',getWeekStartDate(parseInt(subitem)) );
                                        urlFilter.searchParams.set('end-date1',getWeekEndDate(parseInt(subitem)) );

                                        tr+="<td><a href='"+urlFilter.href+"&type=detail'>" +getWeekRange(parseInt(subitem))+"</a></td>";
                                    }
                                    if(subkey==1){
                                        tr+="<td>" +subitem+"</td>";
                                    }
                                ...
                                tr+="<td>" +(item[costKey]/item[goalKey]).toFixed(2)+"$</td>";
                            ...
                            let addTotal = '';
                            metricsArray.forEach(function (item) {
                                addTotal+='<td>'+results[0].totalsForAllResults[item]+'</td>';
                            });

                            tbody.prepend('<tr><td>Total</td><td></td>'
                                +addTotal+
                                '</tr>');
