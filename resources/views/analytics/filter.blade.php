...
if(!empty($query)){
    if(strpos($query,'www.') !==false){
        $filter = 'ga:pagePath=~^'.$query.'..*\,ga:pagePath=~^'.str_replace('www.','',$query).'..*';
    }else{
    ...
    if(is_array($query_path)){
        $urlAdd = '';
        foreach ($query_path as $key =>$part){
            if(!empty($part)){
                if($key > 0){
                    if($key< count($query_path)){
                        $urlAdd .= $part.'/';
                    }else{
                        $urlAdd .= $part;
                    }
                }elseif($key == 0){
                    if(strpos($part,'www.') !==false){
                        $filter = 'ga:pagePath=~^'.$part.'..*\,ga:pagePath=~^'.str_replace('www.','',$part).'..*';
                    }else{
                        $filter = 'ga:pagePath=~^'.$part.'..*\,ga:pagePath=~^'.'www.'.$part.'..*';
                    }
    ...
    if(!empty($urlAdd)){
        $filter .= '\;ga:pagePath=@'.$urlAdd;  //include site
    }
    if(isset($_GET['filterAdd'])){
        $filter.= $_GET['filterAdd'];
    }
}
?>
...
@section('content')
    <div>
        ...
        <h2><?=$h1_title ?? 'Categories filter'?></h2>
        <div>
    ...
    <script>
        ...
        gapi.analytics.ready(function() {
            ...
            function renderUrldata() {
                ...
                // }else{
                //     filter = 'ga:pagePath=@'+urlBack+'\;ga:pagePath!~^tech..*\;ga:pagePath!~^auto..*';
                // }
