                ...
                if($('#compare')[0].checked == true){
                    ...
                    Promise.all([first_query]).then(function(results) {
                        ...
                        if(data1 && data1 !== undefined){
                            data1.forEach(function (item,key) {
                                var url_text = (item[0].length<50) ? item[0]  : item[0].substr(0, 50)+'...';
                                var tr = "<tr><td>"+(key+1)+"</td><td class='url'><a href='"+item[0]+"'>"+url_text+"</a></td><td>"+item[1]+"</td><td>"+item[2]+"</td></tr>";
                                tbody.append(tr);
                            });
                            tbody.prepend('<tr><td></td><td>Total</td><td>'+results[0].totalsForAllResults['ga:pageviews']+'</td><td>'+results[0].totalsForAllResults['ga:uniquePageviews']+'</td></tr>');
                        }
