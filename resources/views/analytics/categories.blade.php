    ...
    <div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered dataTable  " id="backend-table">
                <thead>
                <tr>
                    <th scope="col" data-type="text">#</th>
                    <th scope="col" data-type="text">Path</th>
                    <th scope="col" data-type="digit">Pageviews</th>
                    <th scope="col" data-type="digit">Unique Pageviews</th>
                </tr>
                </thead>
                <tbody>
...
@section('template_scripts')
    ...
    <script>
        var filterAdd = '\;ga:pagePath!@?/view/';
        filterAdd += '\;ga:pagePath!@?/master/';
        filterAdd += '\;ga:pagePath!@catalog/view/';
        $(document).on('click','.url a', function (e) {
            ...
            if((window.location.search)){
                url_go = '<?=$current_route?>/filter'+window.location.search+'&query='+query+'&filterAdd='+filterAdd;
            }else{
                url_go = '<?=$current_route?>/filter?query='+query+'&filterAdd='+filterAdd;
            }
        ...
        gapi.analytics.ready(function() {
            ...
            dateRangeSelectorCompareChanel1.on('change', function(data) {
                ...
                // urlHistoryAdd('end-date1',data["end-date"]);
            ...
            function renderUrldata(view_id,urlBack, key) {
                ...
                var urlArray = urlBack.split("/");
                var filter;

                if((urlBack.indexOf('www.') > -1)){
                    filter = 'ga:pagePath=~^'+urlArray[0]+'..*\,ga:pagePath=~^'+urlArray[0].replace('www.','')+'..*';
                }else{
                    filter = 'ga:pagePath=~^'+urlArray[0]+'..*\,ga:pagePath=~^'+'www.'+urlArray[0]+'..*';
                }

                var urlAdd='';
                for (var i = 1;i <=urlArray.length; i++) {
                    if(urlArray[i] !== undefined && urlArray[i] !== ''){
                ...
                // filter =filter+'\;ga:pagePath=@'+urlAdd;
                // filter ='ga:pagePath=@'+urlAdd;
                if(urlAdd){
                    filter +='\;ga:pagePath=@'+urlAdd;
...
//                    daterange1,
                    ...
                    Promise.all([first_query,second_query]).then(function(results) {
                        ...
                        if(data1 !== undefined){
                            ...
                            tr1.find('.pageviews').text(0);
                            tr1.find('.pageviews-uniq').text(0);
                        }
                        ...
                        if(data2 !== undefined){
                            ...
                            tr2.find('.pageviews').text(0);
                            tr2.find('.pageviews-uniq').text(0);
                        }
